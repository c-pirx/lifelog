...
**Sprawdzenia plików w skryptach wdrożenia idą przez `sudo`.**
`/etc/asystent` ma prawa 750 root:root, a skrypty biegną na koncie `ubuntu`.
Zwykłe `[ -f /etc/asystent/env ]` nie umie wejść do katalogu i zgłasza brak
pliku także wtedy, gdy plik jest. Kosztowało to wymianę `MCP_TOKEN`,
`SESSION_SECRET` i `APP_PASSWORD` przy **każdym** wdrożeniu — zerwany konektor
i hasło, którym nie dało się zalogować.
